...
class MiniWidgetProvider : HomeWidgetProvider() {
  ...
  private fun updateWidget(
    ...
    if (widthPx <= 0 || heightPx <= 0) {
      Log.d(TAG, "MiniWidget size invalid: ${widthPx}x${heightPx}")
  ...
  private fun setImageWithIpcGuard(
    ...
    if (bitmap.byteCount <= MAX_BITMAP_BYTES) {
      ...
      Log.d(TAG, "MiniWidget bitmap inline bytes=${bitmap.byteCount}")
      return
    }
    ...
    try {
      ...
      Log.d(TAG, "MiniWidget bitmap via uri bytes=${bitmap.byteCount}, launcher=${launcherPackage ?: "unknown"}")
    } catch (_: Exception) {
      val scaled = scaleBitmapToLimit(bitmap, MAX_BITMAP_BYTES)
      views.setImageViewBitmap(imageViewId, scaled)
      Log.d(TAG, "MiniWidget bitmap scaled bytes=${scaled.byteCount}")
    }
  }
